...
LastEditTime: 2021-05-14 12:38:24
...
def main(args):
    ...
    val_compounds,val_proteins,val_cpi_labels,val_compoundids=get_all_graph(data.val_set_gnn,data.protein2seq)
    test_compounds,test_proteins,test_cpi_labels,test_compoundids=get_all_graph(data.test_set_gnn,data.protein2seq)
    ...
    loss_model=MultiTaskLoss(2,args.shared_unit_num, args.embedd_dim, data.word_length, 3, 2, 0.5, data.num_nodes,
    ...
    for p in combinations:
        ...
        early_stop=0
        params_list=[]
        params = list(filter(lambda kv: 'shared_unit' in kv[0], loss_model.named_parameters()))

        base_params = list(filter(lambda kv: 'shared_unit' not in  kv[0], loss_model.named_parameters()))
        for k,v in params:
        ...
        model_path='ckl/lr{}_epoch{}_{}_{}_batch{}_slr{}_global_400.pkl'.format(lr_g,args.n_epochs,args.cpi_dataset,args.dti_dataset,batch_size,shared_lr)
        for epoch in range(args.n_epochs):
            ...
            loss_history.append([loss_epoch_cpi.detach().cpu(),loss_epoch_dti.detach().cpu()])
            ...
            test_dti_performance[str(epoch)]=[val_dti_acc, val_dti_roc, val_dti_pre, val_dti_recall,val_dti_aupr]
            test_cpi_performance[str(epoch)]=[val_acc, val_roc, val_pre, val_recall,val_aupr]
            if best_dti_roc < val_dti_roc and best_cpi_roc < val_roc:
                early_stop=0
                best_cpi_roc=val_roc
                best_dti_roc=val_dti_roc
                print('Best performance: CPI:{}, DTI:{}'.format(best_cpi_roc,best_dti_roc))
            ...
            val_dti_log.append([val_dti_acc, val_dti_roc, val_dti_pre, val_dti_recall,val_dti_aupr])
            epochs_his.append(epoch)
        # search_performace[p]['cpi']= best_test_cpi_record
        # search_performace[p]['dti']= best_test_dti_record       
        ...
        test_cpi_acc, test_cpi_roc, test_cpi_pre, test_cpi_recall,test_cpi_aupr = utils.eval_cpi_2(
                    test_cpi_pred, test_cpi_labels)
        test_dti_performance['final']=[test_dti_acc, test_dti_roc, test_dti_pre, test_dti_recall,test_dti_aupr]
        test_cpi_performance['final']=[test_cpi_acc, test_cpi_roc, test_cpi_pre, test_cpi_recall,test_cpi_aupr]
        
        utils.Log_Writer('logs/final_unit{}_dti_{}_sulr{}_lr{}_bs{}_global_400.json'.format(args.negative_sample,args.dti_dataset,shared_lr,lr_g,batch_size),test_dti_performance)
        utils.Log_Writer('logs/final_unit{}_cpi_{}_sulr{}_lr{}_bs{}_global_400.json'.format(args.negative_sample,args.cpi_dataset,shared_lr,lr_g,batch_size),test_cpi_performance)
        print("Test CPI | acc:{:.4f}, roc:{:.4f}, precision:{:.4f}, recall:{:.4f}, aupr:{:.4f}".
                      format( test_cpi_acc, test_cpi_roc, test_cpi_pre, test_cpi_recall,test_cpi_aupr))
        print('Test DTI | acc:{:.4f}, roc:{:.4f}, precision:{:.4f}, recall:{:.4f}, aupr:{:.4f}'.format(
                test_dti_acc, test_dti_roc, test_dti_pre, test_dti_recall,test_dti_aupr))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--cpi_dataset',type=str,default='human',help='dataset used for cpi task')
    parser.add_argument('--dti_dataset',type=str,default='drugcentral',help='dataset used for dti task')
    ### 共用同一个shared unit layer
    parser.add_argument('--shared_unit_num',type=int,default=1,help='the number of shared units')
    parser.add_argument('--embedd_dim',type=int,default=128,help='the dim of embedding')
    ...
    np.savetxt('results/cpi_result.txt',np.array(results_cpi))
    np.savetxt('results/dti_result.txt',np.array(results_dti))
    ...
    print(avg_cpi)
    print('variant score: ')
    print(cpi_df.std(axis=0))
    avg_dti=np.mean(np.array(results_dti),axis=0)
    print('average score: ')
    print(avg_dti)
    print('variant score: ')
    print(dti_df.std(axis=0))
